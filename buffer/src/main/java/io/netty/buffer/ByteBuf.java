...
/**
 * 一个零个或者更多字节的可以随机并且顺序访问的序列。
 * 这个接口为一个或者多个原始类型数组以及ByteBuffer提供一种抽象视图。
 *
 * buffer的创建
 *
 * 推荐使用Unpooled类里的工具方法而不是调用一个独立的构造方法的实现来创建一个新的buffer。
 *
 * 随机访问索引
 *
 * 就像一个普通的原始类型数组，ByteBuf使用
 *<a href="http://en.wikipedia.org/wiki/Zero-based_numbering">从零开始的索引</a>.
 * 这意味着第一个字节的索引总是0，最后一个字节的索引总是capacity - 1。比如说，要迭代
 * 一个buffer的所有字节，你可以做下面这些事，不管它的内部实现。
 *  *
 *  * <pre>
 *  * {@link ByteBuf} buffer = ...;
 ...
 *
 *  顺序访问索引
 *  ByteBuf提供两个指针变量来支持顺序读和写操作 - readerIndex()writerIndex()分别用于读操作和
 *  写操作。下图展示一个buffer怎样被两个指针分成三个部分：
 *
 * <pre>
 *      +-------------------+------------------+------------------+
 ...
 * 可读取的字节（实际内容）
 *
 * 这一段是实际数据被存储的地方。任何以read或skip开头的名字的操作都会获取或者跳过
 * readerIndex的数据，并且根据可读数据的数量增加它。如果读操作的参数也是ByteBuf，
 * 并且没有指定目的地索引，特定buffer的writerIndex也会同步被增加。
 *
 * 如果没有足够的content留下，IndexOutOfBoundsException会被抛出。刚刚被分配，
 * 包裹，复制的buffer的readerIndex的默认值是0。
 *
 * <pre>
